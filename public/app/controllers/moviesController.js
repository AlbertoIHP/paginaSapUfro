...
function homeController($location, $scope, localStorageService, $resource){
    ...
    $scope.obtenerCursos = function()
    {
      ...
      var auth = $resource('http://localhost:4200/cursos');
    ...
    $scope.obtenerCursosTomados = function(){
      var auth = $resource('http://localhost:4200/userTakeCourses');
    ...
    $scope.desincribirCurso = function(curso)
    {
      ...
      var auth = $resource('http://localhost:4200/userTakeCourses/'+idBorrar, {},{
        delete: {
          method: 'DELETE',
          headers: {
            'Content-Type' : contentType
